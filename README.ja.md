...
**Neovim で書く。Gemini がレビューする。どう直すかは自分で決める。**

Visual モードの選択範囲かバッファ全体に `:AiPolish` を実行すると、Gemini の指摘が本文に下線で示される。フローティングウィンドウには、指摘された箇所・最大 3 つの修正案・理由が 1 件ずつ出る。本文が変わるのは、修正案を採用したときだけ。

![デモ: バッファを校正し、AI Polish のポップアップで修正候補を採用する様子](docs/images/demo-ja.gif)

## ai-polish.nvim の特徴

AI の文章ツールには、選択範囲をまるごと書き直し、新旧の見比べを利用者に任せるものが多い。ai-polish.nvim はモデルをレビュアーとして使う。

- **小さく独立した修正。** Gemini には、直す範囲を最小限にし、1 件に 1 つの修正だけを含め、段落を書き直さないよう指示している。1 件ずつすぐに判断できる。
- **変更を適用するのは自分。** `a` で採用、`x` で却下、`]` / `[` で候補を移動する。採用は 1 件ごとに 1 回の undo で戻せる。
- **校正中も編集できる。** 候補はバッファ上の位置に固定し、適用直前に現在のテキストと照合する（[設計メモ](#設計メモ)）。
- **バッファの中で完結する。** チャット画面やサイドウィンドウは開かない。ポップアップは指摘箇所のそばに出る。
- **意図しない送信をしない。** サイズとコストは手元で概算し、大きなリクエストは確認してから送る（[長い文書の扱い](#長い文書の扱い)）。
...
ポップアップを閉じたままバッファを編集してもかまわない。編集で変わった箇所の候補は、誤って適用されずに外れる。
